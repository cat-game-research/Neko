...
namespace SpaceGraphicsToolkit.Atmosphere
{
	...
	[HelpURL(SgtCommon.HelpUrlPrefix + "SgtAtmosphereLightingTex")]
	[AddComponentMenu(SgtCommon.ComponentMenuPrefix + "Atmosphere LightingTex")]
	public class SgtAtmosphereLightingTex : MonoBehaviour
	{
		...
		public float SunsetStart { set { if (sunsetStart != value) { sunsetStart = value; DirtyTexture(); } } get { return sunsetStart; } } [Range(0.0f, 1.0f)] [SerializeField] private float sunsetStart = 0.45f;

		/// <summary>The end point of the sunset/night transition (0 = dark side, 1 = light side).</summary>
		public float SunsetEnd { set { if (sunsetEnd != value) { sunsetEnd = value; DirtyTexture(); } } get { return sunsetEnd; } } [Range(0.0f, 1.0f)] [SerializeField] private float sunsetEnd = 1.0f;

		/// <summary>The sharpness of the sunset red channel transition.</summary>
		public float SunsetSharpnessR { set { if (sunsetSharpnessR != value) { sunsetSharpnessR = value; DirtyTexture(); } } get { return sunsetSharpnessR; } } [SerializeField] private float sunsetSharpnessR = 3.0f;
		...
		{
			var importer = CwHelper.ExportTextureDialog(generatedTexture, "Atmosphere Lighting");
		...
		{
			innerMaterial.SetTexture(_SGT_LightingTex, generatedTexture);
			outerMaterial.SetTexture(_SGT_LightingTex, generatedTexture);
		}
		...
		private void UpdateTexture()
		{
			...
			if (width > 0)
			{
				...
				if (generatedTexture == null)
				{
					generatedTexture = CwHelper.CreateTempTexture2D("Lighting (Generated)", width, 1, TextureFormat.ARGB32);
...
namespace SpaceGraphicsToolkit.Atmosphere
{
	...
	public class SgtAtmosphereLightingTex_Editor : CwEditor
	{
